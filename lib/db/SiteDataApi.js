...
export async function getSiteDataByPageId({ pageId, from }) {
  ...
  if (process.env.NODE_ENV === 'development') {
    const themeResolution = resolveSiteThemeWithSource({
      notionTheme: data?.NOTION_CONFIG?.THEME,
    ...
    console.log(
      ...
      JSON.stringify({
        ...
        configTheme: BLOG.THEME,
        finalTheme: themeResolution.theme,
...
async function convertNotionToSiteData(
  ...
  if (process.env.NODE_ENV === 'development') {
    const themeResolution = resolveSiteThemeWithSource({
      notionTheme: NOTION_CONFIG?.THEME,
      configTheme: BLOG.THEME
    })
    console.log(
      ...
      JSON.stringify({
        ...
        source: themeResolution.source,
        note:
          'URL ?theme has runtime priority; explicit NEXT_PUBLIC_THEME overrides Notion THEME.'
      })
    )
  }
